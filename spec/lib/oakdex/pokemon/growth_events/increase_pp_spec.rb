...
describe Oakdex::Pokemon::GrowthEvents::IncreasePp do
  ...
  describe '#message' do
    it { expect(subject.message)
      .to eq('Please choose a move of Beauty that should increase its PP.') }
  end
  ...
  describe '#execute' do
    it 'adds growth event' do
      expect(pokemon).to receive(:add_growth_event)
        .with(Oakdex::Pokemon::GrowthEvents::IncreaseMovePp,
