...
describe Oakdex::Pokemon::ExperienceGainCalculator do
  let(:fainted) do
    Oakdex::Pokemon.create('Pikachu', level: 10)
  end

  let(:winner) do
    Oakdex::Pokemon.create('Bulbasaur', level: 12)
  end
  ...
  describe '.calculate' do
    ...
    context 'when the fainted pokemon species is different' do
      let(:fainted) do
        Oakdex::Pokemon.create('Charmander', level: 10)
      end
    ...
    context 'when the fainted pokemon has a higher level' do
      let(:fainted) do
        Oakdex::Pokemon.create('Pikachu', level: 15)
      end
    ...
    context 'when the winner pokemon has a higher level' do
      let(:winner) do
        Oakdex::Pokemon.create('Bulbasaur', level: 14)
      end
    ...
    context 'when the winner affection is more than level 1' do
      let(:winner) do
        Oakdex::Pokemon.create('Bulbasaur', level: 12, amie: {
          affection: 254
        })
      end
      it { expect(result).to eq(269) }
    ...
    context 'when the winner is holding lucky egg' do
      let(:winner) do
        Oakdex::Pokemon.create('Bulbasaur', level: 12, item_id: 'Lucky Egg')
      end
      it { expect(result).to eq(403) }
    end
